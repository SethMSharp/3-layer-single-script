...
def initialize_database(fish_db): 
...
def test_database_connect(fish_db):
    db_fresh_start(fish_db)
    db = Singleton()
    db.get_cursor()
    assert 2  == len(db.sql("SELECT * FROM fish;"))

def test_resetting_after_db_creation(fish_db):
    delete_database()
